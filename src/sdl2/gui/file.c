...
void File_CleanFileName(char *pszFileName)
{
	...
	while (len > 2 && pszFileName[--len] == DIR_SEPARATOR)
		pszFileName[len] = '\0';
}
...
void File_AddSlashToEndFileName(char *pszFileName)
{
	...
	if (len != 0)
	{
		if (pszFileName[len-1] != DIR_SEPARATOR)
		{
			pszFileName[len] = DIR_SEPARATOR; /* Must use end slash */
			pszFileName[len+1] = '\0';
		}
	}
}
...
bool File_DoesFileNameEndWithSlash(char *pszFileName)
{
	...
	if (pszFileName[strlen(pszFileName)-1] == DIR_SEPARATOR)
		return true;
...
void File_SplitPath(const char *pSrcFileName, char *pDir, char *pName, char *pExt)
{
	...
	/* Build pathname: */
	ptr1 = strrchr(pSrcFileName, DIR_SEPARATOR);
	if (ptr1)
	{
		strcpy(pName, ptr1+1);
...
char * File_MakePath(const char *pDir, const char *pName, const char *pExt)
{
	...
	len = strlen(filepath);
	if (filepath[len-1] != DIR_SEPARATOR)
	{
		filepath[len++] = DIR_SEPARATOR;
	}
...
void File_MakeAbsoluteName(char *pFileName)
{
	...
	{
		if (pFileName[inpos] == '.' && pFileName[inpos+1] == DIR_SEPARATOR)
		{
			/* Ignore "./" */
			inpos += 2;
		}
		else if (pFileName[inpos] == '.' && pFileName[inpos+1] == 0)
		{
			...
			if (outpos > 1)
				pTempName[outpos - 1] = 0;   /* Remove the last slash, too */
		         ...
		         && (pFileName[inpos+2] == DIR_SEPARATOR || pFileName[inpos+2] == 0))
		{
			/* Handle "../" */
			char *pSlashPos;
			inpos += 2;
			pTempName[outpos - 1] = 0;
			pSlashPos = strrchr(pTempName, DIR_SEPARATOR);
			if (pSlashPos)
			{
				*(pSlashPos + 1) = 0;
				outpos = strlen(pTempName);
		...
		else
		{
			...
			while (pFileName[inpos] != 0 && outpos < FILENAME_MAX)
			{
				...
				if (pFileName[inpos - 1] == DIR_SEPARATOR)
					break;
			}
		}
	}
